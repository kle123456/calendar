    ...
    jQuery.fn.almanac = function (options) {
        ...
        var Almanac = {
            ...
            jcr: function (d) {
                ...
                if (d == '除') jcrjx = {
                    'suited': ['除服', '疗病', '出行', '拆卸', '入宅'],
                    'tapu': ['求官', '上任', '开张', '搬家', '探病']
                };
                if (d == '满') jcrjx = {
                    'suited': ['祈福', '祭祀', '结亲', '开市', '交易'],
                    'tapu': ['服药', '求医', '栽种', '动土', '迁移']
                };
                ...
                for (i in this.sFtv) {
                    if (jQuery.type(this.sFtv[i]) == 'string' && this.sFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
                        if (Number(RegExp.$1) == (m + 1)) {
                            ...
                            if (RegExp.$3 == '*')  _this[Number(RegExp.$2) - 1].color = '#f0f'
                        }
                }

                for (i in this.yFtv) {
                    if (jQuery.type(this.yFtv[i]) == 'string' && this.yFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
                        if (Number(RegExp.$1) == (m + 1)) {
                    ...
                    if (jQuery.type(this.lFtv[i]) == 'string' && this.lFtv[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
                        ...
                        if (tmp1 >= 0 && tmp1 < n) {
                            ...
                            if (tmp2 >= 0 && tmp2 < _this.length) {
                                ...
                                if (RegExp.$3 == '*')  _this[tmp2].color = '#f0f'
                            }
                        }
                    }
                }
             ...
             */
            // setNewFtv: function(){
            //     var ftv = Almanac.sFtv;
            //     console.log(ftv);
            // },
            // 获取营销节点
            getTodayData: function () {
                // $.ajax({
                //     url: 'http://192.168.1.136:8084/calendar',
                //     type: 'GET',
                //     dataType: 'json',
                //     data: {currentDate: '2017-01-18'},
                //     success:function(response) {
                //         var date = response.data.calendars;
                //         date.forEach(function(_date){
                //            var dateArr = _date.calendarDateTime.split("-");
                //            Almanac.yFtv.push(dateArr[1]+dateArr[2]+"  营销");
                //         });
                //         console.log(Almanac.yFtv);
                //         Almanac.drawCld(global.currYear, global.currMonth, global.afterDrawCld);
                //     },
                //     error:function(msg){
                //         alert("我也不知道怎么...T T ");               //执行错误
                //     }
                // });
                $.get("list.json", function (response) {
                    ...
                    date.forEach(function (_date) {
                        var dateArr = _date.calendarDateTime.split("-");
                        Almanac.yFtv.push(dateArr[1] + dateArr[2] + "  " + _date.artileTile);
                    });
                    Almanac.drawCld(global.currYear, global.currMonth, global.afterDrawCld);
                });
            },
            ...
            showAlmanacDetail: function (sObj) {
                ...
                if (sObj.attr('data-solor')) {
                    ...
                    // 显示农历
                    $(".lunar", container).text((cld[d].isLeap ? '闰' : ' ') + this.formatLunarMonth(cld[d].lMonth) + this.formatLunarDay(cld[d].lDay));
                    // 显示干支
            ...
            drawOutline: function (opts) {
                ...
                for (var i = 0; i < 6; i++) {   // 6行
                    for (var j = 0; j < 7; j++) {
                        ...
                        // 重新绑定事件
                        li.on('click', function (event) {   // 单击事件
            ...
            getTodayHeight: function () {
                var li = $("#date-bd").find('li');
                var len = li.length;
                var idx = 0;
                for (var i = 0; i < len; i++) {
                    if (li.eq(i).hasClass('today')) {
                        idx = parseInt(i / 7);
                    }
                }
                ;
        ...
        return this.each(function () {
            ...
            Almanac.getTodayData();
            Almanac.drawCld(Almanac.today.tY(), Almanac.today.tM(), global.afterDrawCld);
            DateSelection.leftX = -Almanac.getTodayHeight() * 6.3;
            DateSelection.rightX = -Almanac.getTodayHeight() * 6.3;
            DateSelection.upTranslateX = Almanac.getTodayHeight() * 6.3;
            ...
            // 左右滑动切换月份,上下滑切换样式
            $("#date-bd").on('touchstart', function (event) {
                // event.preventDefault();
                DateSelection.startPageX = event.originalEvent.touches[0].pageX;
                DateSelection.upStartY = event.originalEvent.touches[0].pageY;
            });
            $("#date-bd").on('touchend', function (event) {
                // event.preventDefault();
                DateSelection.endPageX = event.originalEvent.changedTouches[0].pageX;
                DateSelection.dX = DateSelection.endPageX - DateSelection.startPageX;
                DateSelection.upEndY = event.originalEvent.changedTouches[0].pageY;
                DateSelection.upDY = DateSelection.upEndY - DateSelection.upStartY;
                if (DateSelection.dX < -30) {
                    if ($("#date-bd").hasClass('slide-up')) {
                        DateSelection.leftX -= 6.3;
                        if (DateSelection.leftX <= -37.8) {
                            DateSelection.leftX = 0;
                            DateSelection.rightX = DateSelection.leftX;
                            DateSelection.goNextMonth();
                            $("#date-bd").css({
                                left: '0px'
                            });
                            ...
                            DateSelection.rightX = DateSelection.leftX;
                            $("#date-bd").animate({left: DateSelection.leftX + "rem"});
                        }
                    } else {
                        DateSelection.goNextMonth();
                        DateSelection.upTranslateX = Almanac.getTodayHeight() * 6.3;
                    }
                } else if (DateSelection.dX > 30) {
                    if ($("#date-bd").hasClass('slide-up')) {
                        DateSelection.rightX += 6.3;
                        if (DateSelection.rightX >= 6.3) {
                            DateSelection.rightX = -31.5;
                            DateSelection.leftX = DateSelection.rightX;
                            DateSelection.goPrevMonth();
                            $("#date-bd").css({
                                left: '-31.5rem'
                            });
                            ...
                            DateSelection.leftX = DateSelection.rightX;
                            $("#date-bd").animate({left: DateSelection.rightX + "rem"});
                        }
                    } else {
                        DateSelection.goPrevMonth();
                        DateSelection.upTranslateX = Almanac.getTodayHeight() * 6.3;
                ...
                if (DateSelection.upDY < -40) {
                    $(".left").animate({
                        ...
                        500, function () {
                            ...
                            $("#right").scrollTop(3);
                            $("#date-bd").addClass('slide-up').css({left: -DateSelection.upTranslateX + "rem"});
                            $("#right").removeClass("right").addClass('slide-right');
                        });
                }
            });
            $("#right").scroll(function (event) {
                if ($("#right").scrollTop() <= 0) {
                    setTimeout(function () {
                        $("#right").removeClass("slide-right").addClass('right');
                        $("#date-bd").removeClass("slide-up");
                    }, 400);
                }
            });
            // 返回今天
